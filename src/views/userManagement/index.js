...
export default function EnhancedTable(props) {
  ...
  const filterList = (newFilter) => {
    const textSearchRows = rows.filter(row => { 
      return row.fullName.toLowerCase().includes(newFilter.searchText.toLowerCase()) ||
                ...
                .map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell align="left">{row.winMatch}</TableCell>                     
                      <TableCell align="left">{row.drawMatch}</TableCell>
                      <TableCell align="left">{row.loseMatch}</TableCell>
                      <TableCell align="left">{row.cupCount}</TableCell>
                      <TableCell align="left">{row.roleId.toString() === '1' ? 'user' : 'admin' }</TableCell>
                      <TableCell align="left">{row.state.toString() === '1' ? 'active' : 'block' }</TableCell>
                      <TableCell>
                      <Link to = {`/user-profile-lite/${row.id}`} style = {{textDecoration: "none"}}>
